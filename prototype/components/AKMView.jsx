...
const STATE_CONFIG = {
  ...
  suspended:   { label: 'Suspended', color: 'var(--muted)',   bg: 'var(--surface-2)',  border: 'var(--muted)',  dot: '#8e8a82' },
  refuted:     { label: 'Refuted',   color: 'oklch(0.5 0.18 25)',   bg: 'oklch(0.95 0.06 25)',  border: 'oklch(0.85 0.1 25)',   dot: '#d8493f' },
  closed:      { label: 'Closed',    color: 'var(--muted)',  bg: 'var(--border)',  border: 'var(--muted)',  dot: '#a0a0a0' },
};
...
function MiniRails({ hyps }) {
  ...
  const trackCol = 'var(--muted)';
  const branchCol = 'var(--border)';
      ...
      {hover && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--text-soft)', background: 'var(--surface-2)', padding: '2px 6px', borderRadius: 4 }}>
              {hover.h.id.toUpperCase()}
            </span>
            <StatePill state={hover.h.state} />
            {hover.h.rail > 0 && (
              <span style={{ fontSize: 10, color: 'var(--muted)', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 3 }}>
              ...
              <div style={ak.legend}>
                ...
                <div style={{ ...ak.legendItem, marginLeft: 'auto' }}>
                  <svg width="30" height="10" style={{ overflow: 'visible' }}>
                    <line x1="0" y1="5" x2="12" y2="5" stroke="var(--muted)" strokeWidth="1.3" strokeDasharray="3,3"/>
                    <line x1="12" y1="5" x2="30" y2="5" stroke="var(--muted)" strokeWidth="1.3"/>
                  </svg>
                  Branch rail
                </div>
              </div>
              ...
              {narrativeOpen && (
                narrativeEdit ? (
                  ...
                  <div style={ak.narrative}>
                    {activeLine.narrative || <span style={{ color: 'var(--muted)', fontStyle: 'italic' }}>No narrative yet.</span>}
                  </div>
                )
              )}
            </div>
                ...
                {lineHyps.map((h) => {
                  ...
                  return (
                    <div key={h.id} id={`hyp-card-${h.id}`}
                      style={{
                        ...ak.hypCard,
                        ...(isSel ? { borderColor: cfg.border, background: 'var(--surface)', boxShadow: `0 2px 14px ${cfg.bg}` } : {}),
...
const ak = {
  ...
  title: { fontSize: 26, fontWeight: 400, color: 'var(--text)', fontFamily: 'var(--font-display)', margin: 0, letterSpacing: '-0.01em', whiteSpace: 'nowrap' },
  subtitle: { fontSize: 13, color: 'var(--muted)', fontFamily: 'var(--font-ui)', marginTop: 4 },
  headerStats: { display: 'flex', gap: 24, paddingTop: 6 },
  headerStat: { textAlign: 'right' },
  headerStatNum: { fontSize: 26, fontWeight: 400, color: 'var(--text)', fontFamily: 'var(--font-display)', letterSpacing: '-0.01em', lineHeight: 1 },
  headerStatLabel: { fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-ui)', marginTop: 4, whiteSpace: 'nowrap' },
  body: { display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden', padding: '20px 32px 28px', gap: 16 },

  // Page toggle tabs
  pageTabs: { display: 'flex', gap: 4, padding: '14px 32px 0', borderBottom: '1px solid var(--border)', flexShrink: 0 },
  pageTab: { position: 'relative', fontSize: 13, fontWeight: 600, color: 'var(--muted)', background: 'none', border: 'none', padding: '8px 14px', cursor: 'pointer', fontFamily: 'var(--font-ui)', borderBottom: '2px solid transparent', marginBottom: -1, transition: 'color 0.12s' },
  pageTabActive: { color: 'var(--accent)', borderBottomColor: 'var(--accent)' },

  // Lines panel
  linesPanel: { width: 288, minWidth: 288, background: 'var(--surface)', borderRadius: 12, border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' },
  panelHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px 12px', borderBottom: '1px solid var(--border)', flexShrink: 0 },
  panelTitle: { fontSize: 11, fontWeight: 700, color: 'var(--text-soft)', fontFamily: 'var(--font-ui)', textTransform: 'uppercase', letterSpacing: '0.06em' },
  addBtn: { width: 26, height: 26, borderRadius: 6, border: '1px solid oklch(0.88 0.02 var(--accent-hue,260))', background: 'var(--accent-light)', color: 'var(--accent)', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-ui)', lineHeight: 1 },
  newLineForm: { padding: '12px 14px', borderBottom: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 8 },
  newLineInput: { width: '100%', border: '1px solid oklch(0.88 0.02 var(--accent-hue,260))', borderRadius: 8, padding: '10px 12px', fontSize: 13, fontFamily: 'var(--font-ui)', color: 'var(--text)', resize: 'none', outline: 'none', boxSizing: 'border-box' },
  ...
  cancelBtn: { fontSize: 12, padding: '6px 12px', borderRadius: 6, border: '1px solid var(--border)', background: 'none', color: 'var(--muted)', cursor: 'pointer', fontFamily: 'var(--font-ui)' },
  lineItem: { padding: '14px 18px', borderBottom: '1px solid var(--surface-2)', cursor: 'pointer', transition: 'background 0.12s', borderLeft: '3px solid transparent' },
  lineItemActive: { background: 'var(--accent-light)', borderLeftColor: 'var(--accent)' },
  lineItemTop: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 },
  lineDate: { fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)' },
  lineQuestion: { fontSize: 12.5, fontWeight: 500, color: 'var(--text)', fontFamily: 'var(--font-ui)', lineHeight: 1.5, marginBottom: 8 },
  miniRailsWrap: { padding: '4px 0 6px' },
  lineMeta: { display: 'flex', gap: 12, fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-ui)', marginTop: 4 },
  ...
  // Center panel
  centerPanel: { flex: 1, minWidth: 0, background: 'var(--surface)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'auto', padding: '22px 26px 28px' },
  lineDetailHeader: { display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 18, paddingBottom: 16, borderBottom: '1px solid var(--border)' },
  lineDetailLabel: { fontSize: 10, fontWeight: 600, color: 'var(--muted)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 },
  lineDetailQ: { fontSize: 17, fontWeight: 500, color: 'var(--text)', fontFamily: 'var(--font-ui)', lineHeight: 1.45 },

  // Rails zone
  railsZone: { background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 18px 12px', marginBottom: 24 },
  railsHeaderRow: { display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 10 },
  sectionTitle: { fontSize: 12, fontWeight: 700, color: 'var(--text)', fontFamily: 'var(--font-ui)', textTransform: 'uppercase', letterSpacing: '0.06em' },
  sectionSub: { fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--font-ui)', fontStyle: 'italic', marginLeft: 6 },
  hint: { fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)' },
  legend: { display: 'flex', alignItems: 'center', gap: 16, marginTop: 8, paddingTop: 10, borderTop: '1px solid var(--surface-2)' },
  legendItem: { display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-ui)' },
  legendDot: { width: 8, height: 8, borderRadius: '50%' },

  // Sections
  section: { marginBottom: 24 },
  sectionHeader: { display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 },
  collapseBtn: { display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', padding: 0, color: 'var(--text)', fontFamily: 'var(--font-ui)' },
  ...
  narrative: { fontSize: 13.5, color: 'var(--text)', lineHeight: 1.75, fontFamily: 'var(--font-ui)', whiteSpace: 'pre-wrap', background: 'var(--bg)', borderRadius: 8, padding: '14px 16px' },
  narrativeEdit: { width: '100%', border: '1px solid oklch(0.85 0.04 var(--accent-hue,260))', borderRadius: 8, padding: '12px 14px', fontSize: 14, fontFamily: 'var(--font-ui)', color: 'var(--text)', resize: 'vertical', outline: 'none', lineHeight: 1.7, boxSizing: 'border-box' },

  // Hypothesis cards
  hypList: { display: 'flex', flexDirection: 'column', gap: 10 },
  newHypForm: { marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 8 },
  emptyHyp: { fontSize: 13, color: 'var(--muted)', fontStyle: 'italic', fontFamily: 'var(--font-ui)', padding: '10px 0' },
  hypCard: { border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', cursor: 'pointer', transition: 'all 0.15s', background: 'var(--bg)' },
  hypCardRefuted: { background: 'var(--surface-2)' },
  ...
  branchTag: { fontSize: 10.5, color: 'var(--muted)', fontFamily: 'var(--font-ui)', display: 'inline-flex', alignItems: 'center', gap: 3, fontWeight: 500 },
  hypDesc: { fontSize: 13.5, color: 'var(--text)', fontFamily: 'var(--font-ui)', lineHeight: 1.6 },
  refutationBox: { marginTop: 12, background: 'var(--surface-2)', border: '1px solid oklch(0.88 0.07 25)', borderRadius: 8, padding: '12px 14px' },
  ...
  hypActLabel: { fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-ui)', marginRight: 4 },
  stateBtn: { fontSize: 11, padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'none', color: 'var(--muted)', cursor: 'pointer', fontFamily: 'var(--font-ui)', fontWeight: 500, transition: 'all 0.12s' },
  stateBtnActive: { color: '#fff' },

  // Right panel
  rightPanel: { width: 228, flexShrink: 0, background: 'var(--surface)', borderRadius: 12, border: '1px solid var(--border)', padding: '18px 16px', overflow: 'auto', alignSelf: 'stretch' },
  rpTitle: { fontSize: 11, fontWeight: 700, color: 'var(--text-soft)', fontFamily: 'var(--font-ui)', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.06em' },
  rpSection: { marginBottom: 18 },
  rpLabel: { fontSize: 10, fontWeight: 700, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8, fontFamily: 'var(--font-ui)' },
  rpPaper: { fontSize: 12.5, color: 'oklch(0.3 0.02 var(--accent-hue,260))', fontFamily: 'var(--font-ui)', padding: '6px 0', borderBottom: '1px solid var(--surface-2)', cursor: 'pointer' },
  rpEmpty: { fontSize: 12, color: 'var(--muted)', fontStyle: 'italic', fontFamily: 'var(--font-ui)' },
  rpStatRow: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 },
  rpStatDot: { width: 8, height: 8, borderRadius: '50%', flexShrink: 0 },
  rpStatLabel: { fontSize: 12, color: 'var(--text-soft)', fontFamily: 'var(--font-ui)', flex: 1 },
  rpStatNum: { fontSize: 13, fontWeight: 500, color: 'var(--text)', fontFamily: 'var(--font-display)' },
  rpNote: { fontSize: 12, color: 'var(--text-soft)', fontFamily: 'var(--font-ui)', lineHeight: 1.6, background: 'var(--accent-light)', borderRadius: 8, padding: '10px 12px', border: '1px solid oklch(0.93 0.015 var(--accent-hue,260))' },
  rpMeta: { fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--font-mono)' },
};
